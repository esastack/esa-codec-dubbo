...
public class NettyUtils {
    ...
    public static void setAttachmentsTtfbKey(Map<String, String> attachments, Channel channel) {
        ...
        Attribute<Long> decodeTtfbTime = channel.attr(DubboConstants.DECODE_TTFB_KEY);
        if (decodeTtfbTime.get() != null) {
            attachments.put(DubboConstants.TRACE.TTFB_KEY, String.valueOf(decodeTtfbTime.get()));
        }
        //Dubbo协议收包完成
        Attribute<Long> decodeTtfbCompleteTime = channel.attr(DubboConstants.DECODE_TTFB_COMPLETE_KEY);
        if (decodeTtfbCompleteTime.get() != null) {
            attachments.put(DubboConstants.TRACE.TTFB_COMPLETE_KEY, String.valueOf(decodeTtfbCompleteTime.get()));
        }
    }
    ...
    public static Map<String, String> extractTtfbKey(final Channel channel) {
        ...
        //Dubbo协议收包完成
        final Attribute<Long> decodeTtfbCompleteTime = channel.attr(DubboConstants.DECODE_TTFB_COMPLETE_KEY);
        if (decodeTtfbCompleteTime.get() != null) {
            attachments.put(DubboConstants.TRACE.TTFB_COMPLETE_KEY, String.valueOf(decodeTtfbCompleteTime.get()));
        }
        return attachments;
